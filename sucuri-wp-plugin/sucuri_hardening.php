...
function sucuri_harden_adminuser()
{
    ...
    if(isset($_POST['sucuri_harden_adminuser']) &&
      ($_POST['sucuri_harden_adminuser'] == 'sucuri_harden_adminuser') &&
      $cp == 0)
    {
        if(!isset($_POST['sucuri_harden_adminusernew']))
        {
            $upmsg = sucuri_harden_error("New admin user name not chosen.");
        }
        else
        {
            $_POST['sucuri_harden_adminusernew'] = trim($_POST['sucuri_harden_adminusernew']);
            $_POST['sucuri_harden_adminusernew'] = htmlspecialchars($_POST['sucuri_harden_adminusernew']);

            if(strlen($_POST['sucuri_harden_adminusernew']) < 2)
            {
                $upmsg = sucuri_harden_error("New admin user name not chosen.");
            }
            else if(!preg_match('/^[a-zA-Z0-9_-]+$/', 
                    $_POST['sucuri_harden_adminusernew'], $regs, 
                    PREG_OFFSET_CAPTURE, 0))
            {
                $upmsg = sucuri_harden_error("Invalid user name. Only letters and numbers are allowed.");
            }
            else
            {
                $res = $wpdb->query("UPDATE ".$table_prefix."users ".
                                    "SET user_login = '".
                                    $_POST['sucuri_harden_adminusernew']."'".
                                    "WHERE user_login='admin'");
                $cp = 1;
                $upmsg = sucuri_harden_ok("User name changed to: ".
                                          $_POST['sucuri_harden_adminusernew'].
    ...
    if($cp == 0)
    {
        ...
        echo '<form action="" method="post">'.
             '<input type="hidden" name="wpsucuri-doharden" value="wpsucuri-doharden" />'.
             '<input type="hidden" name="sucuri_harden_adminuser" '.
             'value="sucuri_harden_adminuser" />'.
             '<input type="text" name="sucuri_harden_adminusernew" value="" />'.
...
function __sucuri_scandir($dir, $dofix, &$totalfiles, &$totalwarns)
{
    $dh = opendir($dir);
    if(!$dh)
    {
    ...
    while (($myfile = readdir($dh)) !== false)
    {
        ...
        {
           $p = substr(sprintf('%o', fileperms($dir."/".$myfile)), -3);
           if($p == "777")
           {
               echo "Incorrect permission for directory: ".htmlspecialchars($dir."/".$myfile)." - $p\n";
               $totalwarns++;
               if($dofix == 1)
               {
                  if(chmod($dir."/".$myfile, "0755") === FALSE)
                  {
                     echo "ERROR: Unable to fix permissions for: ".htmlspecialchars($dir."/".$myfile)."\n";
                  }
                  else
                  {
                     echo "OK: Permissions fixed for: ".htmlspecialchars($dir."/".$myfile)."\n";
                  }
              } 
           }
           __sucuri_scandir($dir."/".$myfile, $dofix, $totalfiles, $totalwarns);
        }
        else if(is_link($dir."/".$myfile))
        {
            continue;
        }
        else
        {
           ...
           if($p == "777")
           {
               echo "Incorrect permission for file: ".htmlspecialchars($dir."/".$myfile)." - Permission: $p\n";
               $totalwarns++;
               if($dofix == 1)
               {
                  if(chmod($dir."/".$myfile, "0644") === FALSE)
                  {
                     echo "ERROR: Unable to fix permissions for: ".htmlspecialchars($dir."/".$myfile)."\n";
                  }
                  else
...
function sucuri_harden_perms()
{
    $upmsg = NULL;
    $cp = 1;

    $d1 = substr(sprintf('%o', fileperms(ABSPATH)), -3);
    $d2 = substr(sprintf('%o', fileperms(ABSPATH."/wp-content")), -3);
    $d3 = substr(sprintf('%o', fileperms(ABSPATH."/wp-content/uploads")), -3);
    $f1 = substr(sprintf('%o', fileperms(ABSPATH."/index.php")), -3);
    ...
    $totalwarns = 0;
    $totalfiles = 0;
    $msgf = "Scan all your files for permission issues";
    if(isset($_POST['sucuri_harden_perms']) && 
             $_POST['sucuri_harden_perms'] == 'sucuri_harden_perms')
    {
        $dofix = 0;
        if(isset($_POST['sucuri_harden_fixperms']) &&
           $_POST['sucuri_harden_fixperms'] == 'sucuri_harden_fixperms')
        {
            $dofix = 1;
            if($d1 == "777")
            {
        ...
        __sucuri_scandir(ABSPATH, $dofix, $totalfiles, $totalwarns);
        $scanres = ob_get_contents();
        ob_end_clean();
    ...
    sucuri_harden_status($cp, NULL, 
                         "Permission of the main files look correct",
                         "Permission of some files are too wide, not secure",
                         "It checks whether you have the proper permissions set ".
                         "for all the WordPress files.", $upmsg);
    
    if($totalfiles > 0)
    {
        echo "<br /><br /><b>Total of files scanned: $totalfiles</b><br />";
        if($scanres != NULL)
        {
            echo "<textarea cols=80 rows=6>$scanres</textarea>";
        }
        else
        {
            echo "<i>Looking good. No issues found.</i>";
        }
    }
